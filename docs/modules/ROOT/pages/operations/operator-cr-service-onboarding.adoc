...
If the service is a standard Helm application such as `cert-manager`, `harbor`, `longhorn`, `metallb`, or `loki`, use xref:operations/helm-service-onboarding.adoc[Helm Service Onboarding].
...
. Choose the source repo using xref:operations/service-deployment-patterns.adoc[Service Deployment Patterns]
. Install the operator from the selected source repo
. Create a cluster-local overlay for the operator-managed custom resources
. Ensure the custom-resource overlay depends on the operator installation
. Validate both the operator and the workload custom resources
...
. Create the source object under `applications/overlays/<cluster>/services/sources/`
. Register it from `services/sources/kustomization.yaml`
. Create the operator install `Kustomization` under `applications/overlays/<cluster>/services/fluxcd/<service>.yaml`
. Register it from `services/fluxcd/kustomization.yaml`
...
. Create the database workload from the cluster overlay repo
