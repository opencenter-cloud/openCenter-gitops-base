...
* Configure Helm values with encrypted secrets (see xref:operations/configure-helm-values.adoc[configure-helm-values.adoc])
* Set up observability for secret rotation (see xref:operations/setup-observability.adoc[setup-observability.adoc])
