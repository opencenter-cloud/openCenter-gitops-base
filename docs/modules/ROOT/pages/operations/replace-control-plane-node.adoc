...
. *Hardware failure* - Disk failure, memory errors, network interface issues
. *OS corruption* - Filesystem damage, kernel panics, boot failures
. *Planned maintenance* - OS upgrades requiring full rebuild, hardware replacement
. *Certificate expiration* - Unrecoverable certificate issues requiring node rebuild
. *etcd data corruption* - Database corruption requiring fresh etcd member
...
*Solution:* This is a known issue (https://github.com/kubernetes-sigs/kubespray/issues/12531[#12531]). Ensure `kubelet_secure_addresses` is properly configured with explicit IP addresses in your cluster configuration at deployment time, or run `playbooks/facts.yml` before using `--limit`.
...
. *Try without --limit (reconfigure all nodes):*
+
[,bash]
----
ansible-playbook cluster.yml -b -v
...
. *Always backup etcd* before performing control plane maintenance
+
[,bash]
----
...
. *Maintain odd number of etcd members* (3, 5, or 7 recommended)
 ** 3 nodes: tolerates 1 failure
 ** 5 nodes: tolerates 2 failures
 ** 7 nodes: tolerates 3 failures
. *Never replace multiple control plane nodes simultaneously*
. *Document your cluster configuration* including custom variables and hardening settings
. *Keep Kubespray version consistent* across all operations
. *Monitor etcd health* during and after the replacement
+
[,bash]
----
...
. *Verify backups* are recent and restorable before starting maintenance
