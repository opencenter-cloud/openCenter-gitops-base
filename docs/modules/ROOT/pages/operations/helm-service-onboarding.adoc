...
If the service is installed through OLM, use xref:operations/olm-service-onboarding.adoc[OLM Service Onboarding] instead.

If the operator is installed with Helm and the cluster overlay repo must create custom resources such as `Kafka` or `postgresql`, use xref:operations/operator-cr-service-onboarding.adoc[Operator CR Service Onboarding] instead.
...
. Decide whether the cluster should consume the community repo or the enterprise repo
. Inspect the base service `HelmRelease` to understand how overrides are accepted
. Create the cluster overlay directory structure for the service
. Add the Flux source object in `services/sources/`
. Add the install `Kustomization` in `services/fluxcd/`
. Add cluster-local values, secrets, and manifests in `services/<service>/`
. Validate Flux reconciliation and the resulting workload

For the community versus enterprise source pattern, use xref:operations/service-deployment-patterns.adoc[Service Deployment Patterns].
...
Store sensitive values with SOPS. See xref:operations/manage-secrets.adoc[Manage Secrets with SOPS] and xref:../reference/sops-configuration.adoc[SOPS Configuration].
...
For service-specific guidance, see xref:operations/services/cert-manager.adoc[Cert-manager Configuration Guide].
