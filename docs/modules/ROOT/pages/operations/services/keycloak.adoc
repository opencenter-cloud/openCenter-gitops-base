...
. `00-postgres/` creates the backing PostgreSQL resources
. `10-operator/` creates the OLM `OperatorGroup` and `Subscription`
. OLM creates an `InstallPlan`
. Because `installPlanApproval` is set to `Manual`, the pending `InstallPlan` must be approved from the cluster
. After approval, OLM installs the Keycloak operator
. `20-keycloak/` creates the `Keycloak` custom resource
. `30-oidc-rbac/` can add optional RBAC resources
...
* xref:../../reference/services/keycloak.adoc[Keycloak Service Reference]
