...
. Manages a shared VIP that floats between control plane nodes
. Uses ARP to announce the VIP on the network
. Provides automatic failover if the active node becomes unavailable
. Binds to a specific network interface for VIP management
...
. Verify the VIP is not in the DHCP allocation pool
. Check that `vrrp_enabled = true` and `use_octavia = false`
. Ensure the network interface name (`cni_iface`) is correct
. Verify no firewall rules are blocking ARP traffic
. Check if quorum is broken. Like 2 nodes down out of the total 3.

=== Kube-VIP Pods Not Starting

. Check kubespray deployment logs
. Verify the interface exists on control plane nodes: `ip link show`
. Check the kube-vip container logs directly on the control plane nodes.
...
| Use Case
| Bare metal, on-prem, cost-sensitive
| Cloud environments with LBaaS

| Mutual Exclusivity
| Cannot use with Octavia
| Cannot use with kube-vip
|===
...
. *IP Planning*: Always reserve the VIP outside your DHCP/allocation pool
. *Interface Verification*: Confirm the network interface name before deployment
. *Master Count*: Deploy 1 or 3 control plane nodes (avoid 2 for quorum)
. *Service Load Balancing*: Use MetalLB for service-type LoadBalancer, not kube-vip
. *Monitoring*: Monitor kube-vip pod health and VIP accessibility
. *Documentation*: Document your VIP and reserved IP ranges
...
* https://kube-vip.io/[Kube-VIP Documentation]
* https://github.com/kubernetes-sigs/kubespray/blob/master/docs/kube-vip.md[Kubespray Kube-VIP Integration]
