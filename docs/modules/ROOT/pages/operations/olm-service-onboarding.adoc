...
If the operator is installed with `HelmRelease` and the cluster overlay later creates custom resources such as `Kafka` or `postgresql`, use xref:operations/operator-cr-service-onboarding.adoc[Operator CR Service Onboarding] instead.
...
. Add the source object and install `Kustomization` in the cluster overlay repo
...
. Create the install `Kustomization` under `applications/overlays/<cluster>/services/fluxcd/<service>.yaml`
...
. Reconcile the `Keycloak` custom resource
...
For service-specific guidance, see xref:operations/services/keycloak.adoc[Keycloak Configuration Guide].
...
* xref:operations/services/keycloak.adoc[Keycloak Configuration Guide]
