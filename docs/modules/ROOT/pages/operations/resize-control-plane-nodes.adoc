...
. *Set up the environment variables and cluster access.*
...
. **Update the flavor in the main.tf **
...
. *Apply the change one node at a time*
+
...
. **Repeat steps 6 and 7 on the remaining control plane nodes.

== Verification

Steps to confirm the issue has been resolved:

. *Verify resource utilization on nodes*
...
. *Cluster not responding to kubectl commands*
+
