...
This guide is about *values layering only*. For end-to-end service onboarding in a cluster overlay repo, use xref:operations/helm-service-onboarding.adoc[Helm Service Onboarding].
...
. *Base values* in `applications/base/services/<service>/helm-values/values-<version>.yaml`
. *Base Secret generation* in the service `kustomization.yaml`
. *Optional override Secret reference* in the service `helmrelease.yaml`
...
* xref:../concepts/enterprise-components.adoc[Enterprise Components]
