...
It does not try to re-explain the full GitOps workflow. For that, use xref:../concepts/gitops-workflow.adoc[GitOps Workflow].
...
| string
| No
| Secret containing SSH key or credentials
|===
...
| duration
| Yes
| Reconciliation interval (typically `5m`)

| `timeout`
| duration
| Yes
| Operation timeout (typically `10m`)
...
| object
| Yes
| Reference to HelmRepository or GitRepository

| `install.remediation.retries`
| int
| No
| Install retry count (default: 3)

| `install.remediation.remediateLastFailure`
| bool
| No
| Retry last failure (default: true)

| `upgrade.remediation.retries`
| int
| No
| Upgrade retry count (default: 0)

| `upgrade.remediation.remediateLastFailure`
| bool
| No
| Retry last upgrade failure (default: false)

| `valuesFrom`
| array
| No
| List of value sources (Secrets or ConfigMaps)
|===
...
| array
| No
| List of Kustomizations to wait for
...
| bool
| No
| Delete resources removed from Git (default: false)

| `healthChecks`
| array
| No
| Resources to check before marking ready
|===
...
| GitRepository
| `15m`
| Common cluster-repo source polling interval

| HelmRepository
| `1h`
| Common chart source polling interval

| HelmRelease
| `5m`
| Common service reconciliation interval

| Kustomization
| `5m`
| Common install reconciliation interval
|===
