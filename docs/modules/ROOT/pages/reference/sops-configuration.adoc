...
| `encrypted_regex`
| Regular expression matching YAML keys to encrypt
|===
...
| `decryption.secretRef.name`
| Secret containing age private key
|===
...
. *Consistent .sops.yaml placement* - Place at repository root or cluster root
. *Path-specific rules* - Use path_regex to match directory structure
. *Document encryption rules* - Comment .sops.yaml with rule explanations

=== Operational

. *Test decryption* - Verify FluxCD can decrypt before committing
. *Monitor decryption failures* - Alert on Kustomization decryption errors
. *Audit encrypted files* - Regularly review what's encrypted
. *Validate encryption* - Ensure no plaintext secrets in Git history
