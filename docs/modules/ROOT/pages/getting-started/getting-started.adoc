...
This tutorial uses the current *community repo* onboarding pattern. For the broader decision flow and other deployment models, see xref:../operations/service-deployment-patterns.adoc[Service Deployment Patterns] and xref:../operations/helm-service-onboarding.adoc[Helm Service Onboarding].
...
. Flux creates the cert-manager namespace
. Flux creates the HelmRepository source (Jetstack)
. Flux generates the values Secret from the file
. Flux creates the HelmRelease
. Helm-controller installs cert-manager chart
. Pods start running
...
* xref:../operations/configure-helm-values.adoc[Configure Helm Values] - Override base values for your cluster
* xref:../operations/manage-secrets.adoc[Manage Secrets with SOPS] - Encrypt sensitive configuration

*Deploy More Services:*

* xref:../operations/add-helm-service-to-community-repo.adoc[Add a Helm Service to the Community Repo] - Add a new shared Helm-based service to the community repo
* link:../../README.md#available-applications[Available Applications] - Browse available services and versions

*Advanced Topics:*

* xref:../operations/configure-gateway.adoc[Configure Gateway API] - Set up ingress routing
* xref:../operations/setup-observability.adoc[Setup Observability] - Deploy monitoring stack
* xref:../operations/troubleshoot-flux.adoc[Troubleshoot Flux] - Debug reconciliation issues

*Understand the Architecture:*

* xref:../concepts/gitops-workflow.adoc[GitOps Workflow] - How FluxCD manages deployments
* xref:../concepts/three-tier-values.adoc[Base, Override, and Enterprise Values] - Why we use this layering model
* xref:../concepts/architecture.adoc[Architecture Overview] - System design decisions
