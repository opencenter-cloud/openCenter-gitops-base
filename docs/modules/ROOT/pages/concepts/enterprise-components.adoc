...
. adds the enterprise Helm source
. removes the public upstream `HelmRepository`
. patches the base `HelmRelease` to use the private source and enterprise values Secret
