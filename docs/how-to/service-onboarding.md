...
Cluster overlays let you layer **environment-specific configurations** on top of shared manifests from the [base repository](https://github.com/opencenter-cloud/openCenter-gitops-base), or on top of a private enterprise repo that imports that base when enterprise-specific artifact rewrites are required.

---

## How It Works

The following diagram shows how the **cluster repository**, **base repository**, optional **enterprise repository**, and **Flux** work together to deploy services into your Kubernetes cluster.
...
flowchart LR
  ...
  B -- Base manifests --> C
  E -- Enterprise install overlays --> C
  C -- Deploys merged manifests --> D[Kubernetes Cluster Resources]
```

- **Base Repo** – Holds reusable and shared manifests common across clusters.  
- **Enterprise Repo** – Optionally imports the base and applies private source or image rewrites.  
- **Cluster Repo** – Contains environment-specific configurations, service activation, and cluster-local overrides.  
...
1. Access to:
   - Your **cluster Git repository**  
   - The [openCenter base Git repository](https://github.com/opencenter-cloud/openCenter-gitops-base)
2. Local clones of:
...
4. Git credentials that allow Flux bootstrap to write bootstrap manifests back to your cluster repository.

   If your organization uses SSH deploy keys, create one with:
   ...
   **Settings → Deploy Keys**, granting **write** access.  
   If your organization uses another authentication method, adapt the bootstrap command in the next step accordingly.
...
Adapt the repository URL and authentication flags to match the access model for your cluster repository.
...
2. Copy its directory `applications/overlays/stage-cluster` into your **cluster repo**.  
...
## Step 5: Use HTTPS for the Base Repository

Use HTTPS in the `GitRepository` definition:
...
Use SSH keys or authenticated HTTPS credentials only for private sources, such as the private enterprise repository.
...
3. Ensure the Git credentials and SOPS decryption secret required by the target environment exist before merge.  
