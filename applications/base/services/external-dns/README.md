...
This directory contains the **base manifests** for deploying [ExternalDNS](https://github.com/kubernetes-sigs/external-dns), a controller that synchronizes exposed Kubernetes Services and Ingresses with DNS providers.  
...
- Deployed to `external-dns` namespace.  
- Cluster-specific overrides are supplied via the `external-dns-values-override` Secret (optional).  
